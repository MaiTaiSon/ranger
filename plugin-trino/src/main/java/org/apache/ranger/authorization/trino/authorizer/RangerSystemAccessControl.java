...
public class RangerSystemAccessControl
  ...
  public void checkCanShowTables(SystemSecurityContext context, CatalogSchemaName schema) {
    if (!hasPermission(createResource(schema), context.getIdentity(), TrinoAccessType.SHOW)) {
      LOG.debug("RangerSystemAccessControl.checkCanShowTables(" + schema.getSchemaName() + ") denied");
  ...
  public void checkCanShowCreateTable(SystemSecurityContext context, CatalogSchemaTableName table) {
    if (!hasPermission(createResource(table), context.getIdentity(), TrinoAccessType.SHOW)) {
      LOG.debug("RangerSystemAccessControl.checkCanShowTables(" + table.getSchemaTableName() + ") denied");
  ...
  public void checkCanUpdateTableColumns(SystemSecurityContext securityContext, CatalogSchemaTableName table, Set<String> updatedColumnNames) {
    for (RangerTrinoResource res : createResource(table, updatedColumnNames)) {
      if (!hasPermission(res, securityContext.getIdentity(), TrinoAccessType.EXECUTE)) {
  ...
  public boolean canExecuteFunction(SystemSecurityContext systemSecurityContext, CatalogSchemaRoutineName functionName) {
    if (!hasPermission(createFunctionResource(functionName.getRoutineName()), systemSecurityContext.getIdentity(), TrinoAccessType.EXECUTE)) {
      LOG.debug("RangerSystemAccessControl.checkCanExecuteFunction(" + functionName.getRoutineName() + ") denied");
      AccessDeniedException.denyExecuteFunction(functionName.getRoutineName());
    }
    return true;
  }
